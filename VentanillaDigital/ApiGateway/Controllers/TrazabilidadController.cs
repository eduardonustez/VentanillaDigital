...
namespace ApiGateway.Controllers
{
    ...
    public class TrazabilidadController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("CrearExcepcion")]
        [ProducesResponseType(StatusCodes.Status204NoContent)]
        public async Task<IActionResult> CrearExcepcion(ErrorModel model)
        {
            var serviceResponse = await _httpClientHelper.ConsumirServicioRest($"{uriAPI}/Trazabilidad/CrearExcepcion",
             HttpMethod.Post, model);
            var res = await serviceResponse.Content.ReadAsStringAsync();
            if (serviceResponse.StatusCode == HttpStatusCode.NoContent)
            {
