...
namespace ApiGateway.Controllers
{
    ...
    public class ConsultaController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("ObtenerActa/{notariaId}/{fechaTramite}/{email}/{tramiteId}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<ActionResult<string>> ObtenerActa(string notariaId, string fechaTramite, string email, string tramiteId)
        {
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<ActionResult<string>> ObtenerNotarias()
        {
            var serviceResponse = await _httpClientHelper.ConsumirServicioRest($"{uriAPI}/Notaria/ObtenerNotarias/",
             HttpMethod.Get, "");
            var res = await serviceResponse.Content.ReadAsStringAsync();
            if (serviceResponse.StatusCode == HttpStatusCode.OK)
            {
