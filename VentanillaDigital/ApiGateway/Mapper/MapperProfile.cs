...
namespace ApiGateway.Mapper
{
    public class MapperProfile : Profile
    {
        public MapperProfile()
        {
            ...
            CreateMap<Certificate, CertificadoDTO>()
             .ForMember(target => target.IdCertificado, options => options.MapFrom(source => source.CertificateId))
             .ForMember(target => target.Descripcion, options => options.MapFrom(source => source.Description))
             .ForMember(target => target.Estado, options => options.MapFrom(source => source.State))
             .ForMember(target => target.ValidoDesde, options => options.MapFrom
                  (source => string.IsNullOrWhiteSpace(source.ValidFrom) ? source.ValidFrom : Convert.ToDateTime(source.ValidFrom).ToShortDateString()))
             .ForMember(target => target.ValidoHasta, options => options.MapFrom
                (source => string.IsNullOrWhiteSpace(source.ValidTo) ? source.ValidTo : Convert.ToDateTime(source.ValidTo).ToShortDateString()))
            .PreserveReferences().ReverseMap();
