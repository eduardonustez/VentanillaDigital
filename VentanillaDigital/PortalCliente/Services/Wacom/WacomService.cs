...
namespace PortalCliente.Services.Wacom
{
    public class WacomService : IWacomService, IDisposable
    {
        ...
        public async Task Initialize(bool pantallaInicio=true)
        {
            ...
            if (EstadoServicio == EstadoServicio.DispositivoConectado && pantallaInicio)
            {
        ...
        #region JSInvokable
        [JSInvokable]
        public async Task Reinicializar()
        {
            await Initialize();
        }
        ...
        {
            return await JSRuntime.InvokeAsync<string> ("generateImage", alto, ancho, grosor);
        }
