...
async function ensureDCAReady(attempts) {
        ...
        .then(function (message) {
            if (message) {
                ...
                return new Promise((resolve) => setTimeout(() => resolve(), 250))
                    .then(function () {
                        return ensureDCAReady(attempts + 1);
                    });
            }
            else {
                return false;
...
function connect() {
    ...
    return ensureDCAReady(0)
        .then(function (message) {
            ...
            return intf.connect(m_usbDevices[0], false);
        }).then(function (message) {
            if (0 != message.value) {
...
function onDCAtimeout() {
    ...
    serviceHelper.invokeMethodAsync('Reinicializar')
        .then(disconnect);
}
...
function findRatio(points) {
    ...
    for (var i = 0; i < points.length; i++) {
        ...
        if (isDown2) {
            if (myArea == undefined) {
                myArea = new Rectangle(nextPoint.x, nextPoint.y,
                    0, 0);
            }
...
function generateImage(height, width, lineWidth) {
    var signatureCanvas = document.createElement("canvas");
    signatureCanvas.id = "signatureCanvas";

    var rat = findRatio(m_penData);
    signatureCanvas.height = Math.min(height*rat, m_capability.tabletMaxY);
    signatureCanvas.width = Math.min(width*rat, m_capability.tabletMaxX);
    ...
    return createImageBitmap(signatureCanvas, m_actArea.x, m_actArea.y, m_actArea.width, m_actArea.height)
        .then(function (img) {
            ...
            returnCtx.drawImage(img, (width - m_actArea.width) / 2, (height - m_actArea.height) / 2);
